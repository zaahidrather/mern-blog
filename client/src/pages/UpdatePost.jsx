...
export default function UpdatePost() {
	...
	useEffect(() => {
		async function fetchPost() {
			...
			try {
				...
				setOriginalPost(post); // Keep for comparison
				setPostData({
					// Fill the form state
					title: post.title,
	...
	async function handleSubmit(e) {
		...
		Object.keys(postData).forEach((key) => {
			if (postData[key] !== originalPost[key] && postData[key] !== '') {
				dataToSend[key] = postData[key];
			}
		});

		if (Array.from(Object.entries(dataToSend)).length === 0) {
			dispatch(updatePostFailure('No changes made'));
			return;
		}
	...
	return (
		<div className="mx-auto min-h-screen max-w-3xl p-3">
			<h1 className="my-7 text-center text-3xl font-semibold">Update a post</h1>
					...
					<Field>
						...
						<div className="relative h-40 w-40">
							{isUploading ? (
								...
								originalPost?.image && (
									<img src={postData.image} alt="" className="h-full w-full object-cover" />
								)
							)}
						</div>
						{uploadError && (
